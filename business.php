...
if(isset($_GET["business"]) && isset($_GET["discount"])){

     $query = "UPDATE business SET hits = hits + 1 WHERE bid = {$_GET["business"]}";
     $result = mysqli_query($link, $query);
     $query = "UPDATE discount SET hits = hits + 1 WHERE did = {$_GET["discount"]}";
     $result = mysqli_query($link, $query);
...
$query = "SELECT bname,bemail,ph1,floor,mall,city from business where bid={$_GET['business']}";
$result = mysqli_query($link, $query);
$row = mysqli_fetch_assoc($result);
echo "<strong>".$row['bname']."</strong>".",Floor:".$row["floor"].",".$row["mall"]." mall,".$row["city"]."<br/>";
echo "<strong>Contact: </strong>".$row["bemail"]." ,".$row["ph1"]."<br/>";
?>
...
<center><span style="display:block;font-size:20px;margin-top:0%;padding-top:2%;font-family:verdana;color:#fff;">Discounts and Offers from the store.  </span></center>
...
$query = "SELECT dname,ddesc,cname from discount,category where bid={$_GET["business"]} and discount.cid=category.cid";
$result = mysqli_query($link, $query);
while($userdata = mysqli_fetch_assoc($result)) {
  echo '<div class="row" style="border:dashed;padding:5px;">';
  echo '<div class="col-md-4" style="color:#d7756c;"><h3>'.$userdata["dname"].'</h3></div>';
  echo '<div class="col-md-4" style=""><h3>'.$userdata["ddesc"].'</h3></div>';
  echo '<div class="col-md-4" style="text-align:right;"><span class="btn btn-default btn-lg">'.$userdata["cname"].'</span></div>
</div>
<hr>';
}
